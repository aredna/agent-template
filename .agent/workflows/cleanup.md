...
- NEVER: `git worktree prune`, `pkill -9 {RUNTIME}` (kills ALL), `rm -rf ../*-wt-*`, remove unmerged branches

## 1. Find Orphaned Processes
```bash
pgrep -af "dev|serve" | grep -v grep
pgrep -af "{RUNTIME}" | grep -v grep
lsof -i :{DEFAULT_PORT} 2>/dev/null
```
